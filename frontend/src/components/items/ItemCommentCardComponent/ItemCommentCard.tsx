...
import type { ItemComment, ItemCommentReportPayload } from '@/types/item.types';
import type { ForumReportReason } from '@/types/forum.types';
...
interface ItemCommentCardProps {
  ...
  isBusy?: boolean;
  onUpdate?: (commentId: number, content: string) => Promise<{ success: boolean; message?: string }>;
  onDelete?: (commentId: number) => Promise<{ success: boolean; message?: string }>;
  onReport?: (commentId: number, payload: ItemCommentReportPayload) => Promise<{ success: boolean; message?: string }>;
}

export default function ItemCommentCard({
  comment,
  currentUserId,
  currentUserRole,
  ...
  const canManage = canManageComment(comment, currentUserId, currentUserRole);
  const canReport = isAuthenticated && currentUserId !== undefined && comment.user_id !== currentUserId;
  const [isEditing, setIsEditing] = useState(false);
  const [isReporting, setIsReporting] = useState(false);
  const [draftContent, setDraftContent] = useState(comment.content);
  const [reportData, setReportData] = useState<ItemCommentReportPayload>({ reason: 'inappropriate', details: '' });
  ...
  const handleEditSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const result = await onUpdate(comment.id, draftContent.trim());
    if (!result.success) {
      setActionError(result.message ?? 'Impossible de modifier le commentaire.');
      return;
    }
    setIsEditing(false);
  };
  ...
  const handleDelete = async () => {
    ...
    const confirmed = window.confirm('Supprimer ce commentaire ? Cette action est definitive.');
    if (!confirmed) return;

    const result = await onDelete(comment.id);
    if (!result.success) {
      setActionError(result.message ?? 'Impossible de supprimer le commentaire.');
    }
  };
  ...
  return (
    <article className={styles.card}>
      <aside className={styles.author}>
        <div className={styles.avatar}>{comment.author?.nom?.charAt(0).toUpperCase() ?? '?'}</div>
        <strong>{comment.author?.nom ?? 'Aventurier'}</strong>
        {comment.author?.pseudo_gw2 && <span>{comment.author.pseudo_gw2}</span>}
      </aside>
      <div className={styles.body}>
        ...
        {isEditing ? (
          <form className={styles.editForm} onSubmit={handleEditSubmit} noValidate>
            <textarea value={draftContent} onChange={handleDraftChange} className={styles.editTextarea} minLength={2} required />
            ...
            <div className={styles.editActions}>
              ...
              <button type="submit" className={styles.saveButton} disabled={isBusy}>Enregistrer</button>
            </div>
          </form>
        ) : (
          <>
            {isReporting && (
              <form className={styles.reportForm} onSubmit={handleReportSubmit} noValidate>
                <label className={styles.reportLabel} htmlFor={`comment-report-reason-${comment.id}`}>
                ...
                <div className={styles.editActions}>
                  ...
                  <button type="submit" className={styles.saveButton} disabled={isBusy}>Envoyer</button>
                </div>
              </form>
            )}
